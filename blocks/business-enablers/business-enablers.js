 ...
 * Maps authored items to three specific component designs.
 */
export default function decorate(block) {
    ...
    // Parse authored items and map them to the 3 slots
    // items have fields: type, tabTitle, tabDescription, desktopImage, mobileImage, imageAlt, buttonLabel, buttonLink
    ...
    // Render exactly 3 tabs, matching buttons
    [0, 1, 2].forEach(index => {
    ...
    /**
     * Helper to render the internal layout based on type
     */
        ...
        <div class="our-capabilities-container">
          <div class="row align-items-center">
            ...
            <div class="col-lg-6 our-capabilities-image-wrap">
              <div class="our-capabilities-picture"></div>
            </div>
          </div>
        </div>
      `;
            const picWrap = targetEl.querySelector(".our-capabilities-picture");
            if (desktopPic) {
                const d = desktopPic.cloneNode(true);
                d.classList.add("d-none", "d-md-block");
                if (d.querySelector("img")) d.querySelector("img").alt = imageAlt;
                picWrap.append(d);
            }
            if (mobilePic) {
                const m = mobilePic.cloneNode(true);
                m.classList.add("d-block", "d-md-none");
                if (m.querySelector("img")) m.querySelector("img").alt = imageAlt;
                picWrap.append(m);
            }
        } else if (type === "cards") {
            // Structure of 'cards-and-image' (Simplified for tab use)
            targetEl.innerHTML = `
        <div class="cards-image-container">
          <div class="cards-image-intro row align-items-center mb-4">
            <div class="col-lg-6 cards-image-intro-text">
              <h2 class="cards-image-title">${title}</h2>
              <div class="cards-image-description">${contentHTML}</div>
            </div>
            <div class="col-lg-6 cards-image-intro-img">
              <div class="cards-image-picture-wrap"></div>
            </div>
          </div>
        </div>
      `;
            const picWrap = targetEl.querySelector(".cards-image-picture-wrap");
            if (desktopPic) {
                const d = desktopPic.cloneNode(true);
                if (d.querySelector("img")) d.querySelector("img").alt = imageAlt;
                picWrap.append(d);
            ...
            // 'image-richtext'
            targetEl.innerHTML = `
        <div class="image-richtext-container">
          <div class="row align-items-center">
            <div class="col-lg-6 image-richtext-content-col">
              <div class="image-richtext-text">${contentHTML}</div>
            </div>
            <div class="col-lg-6 image-richtext-img-col">
              <div class="image-richtext-picture-wrap"></div>
            </div>
          </div>
        </div>
      `;
            if (desktopPic) {
                const d = desktopPic.cloneNode(true);
                if (d.querySelector("img")) d.querySelector("img").alt = imageAlt;
                targetEl.querySelector(".image-richtext-picture-wrap").append(d);
            }
        }
    }
}
